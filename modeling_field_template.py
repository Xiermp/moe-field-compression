...
field_mode (2026-09-06, 10.9):
  - preact (по умолчанию, как раньше): один проход, смесь координат
    c(z)=z@C до нелинейности; SwiGLU даёт кросс-члены z1z2(g1*u2+g2*u1),
    фит компенсирует их лишь частично;
  - postact: per-expert ветки - общий пре-бейс x@wgud^T, низкоранговые
    дельты и НЕЛИНЕЙНОСТЬ по каждому выбранному эксперту, выход =
    sum_e z_e*FFN_e(x) (нулевой вклад кросс-членов; ~+1 down-GEMM на токен;
    при top_k=1 композиции совпадают точно). Режим читается из
    config.field.field_mode и пишется пайплайном (--field-mode).

banks (2026-09-08, UPDATE-12):
  - config.field.banks >= 2: второй банк координат (U2gu,V2gu,C2gu) и
    (U2dn,V2dn,C2dn) - дельта-пространство удваивается (фикс H2 "ёмкость
    низкого ранга" из IRON-диагностики). Банк 2 входит АДДИТИВНО в те же
    места (gu до нелинейности, dn после), при нулевых bank2-весах выход
    бит-в-бит равен одному банку; артефакты без ключа banks работают как
    раньше.

core (2026-09-08, UPDATE-13):
  - config.field.core == "dense" (Tucker-2): координаты C становятся
    ПЛОТНЫМ ядром (n_exp, r, r) - дельта эксперта e = U @ G_e @ Vᵀ,
    забирает cross-члены общего базиса (иниц. из whbank_build, шаг 0
    без слепого старта). Смесь ядер в preact: G_mix = sum_e z_e G_e
    (два батч-эйнсама, без per-expert цикла); в postact - ядро
    выбранного эксперта. Банк 2 остаётся диагональным. Значение по
    умолчанию (ключ отсутствует) - "diag": прежнее поведение бит-в-бит.

u_mode (2026-09-10, 13.5):
  - config.field.u_mode: per-экспертные дельты на ВТОРОЙ матрице
    down-ветки (Udn): du_e = duAdn[e] @ duBdn[e]^T (rank1/rank4,
    u_rank = a) или duEdn[e] (r, d) в режиме "full". Терм добавляется
    к выходу ПОСЛЕ нелинейности как top-k-смесь
    sum_j z_j * ((h@Vdn) @ du_ej) - новые ВЫХОДНЫЕ направления
    (fig12/fig14: словарь комбинаций монотонно предсказывает качество,
    Spearman -0.94; novelty=0 у координатных коннекторов).
  - LoRA-старт: duBdn/duEdn нулевые -> шаг 0 бит-в-бит равен u_mode none;
    ключи отсутствуют в config.field (старые артефакты) - тоже none.
  - работает в ОБЕИХ композициях (preact: hv = h@Vdn общий, du-смесь
    отдельным top-k-циклом; postact: внутри per-expert цикла).
    Бюджет: rank-a +E*(r+d)*a параметров на блок, full +E*r*d
    (~top_k*(r*a + a*d) MAC/токен в rank-a - копейки против FFN).
"""
import inspect
